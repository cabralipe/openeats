...
class DeliveryViewSet(viewsets.ModelViewSet):
    ...
    def sign(self, request, pk=None):
        if not hasattr(request.user, 'is_nutritionist') or not request.user.is_nutritionist:
            raise ValidationError('Apenas contas de nutricionista podem assinar entregas.')

        delivery = self.get_object()
        if delivery.status not in [Delivery.Status.CONFERRED, Delivery.Status.FINALIZED]:
            raise ValidationError('Apenas entregas conferidas ou já finalizadas podem ser assinadas.')

        signature_data = request.data.get('signature_data')

        if not signature_data or not signature_data.startswith('data:image/'):
            raise ValidationError({'signature_data': 'Assinatura inválida.'})

        with transaction.atomic():
            DeliveryNutritionistSignature.objects.create(
                delivery=delivery,
                name=request.user.name,
                crn=getattr(request.user, 'crn', ''),
                function_role=getattr(request.user, 'function_role', 'Nutricionista'),
                signature_data=signature_data
            )
            if delivery.status != Delivery.Status.FINALIZED:
                delivery.status = Delivery.Status.FINALIZED
